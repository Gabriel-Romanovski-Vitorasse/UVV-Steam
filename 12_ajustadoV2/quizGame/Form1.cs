...
namespace quizGame
{
    public partial class Form1 : Form
    {
        ...
        {
            // Verifica se a ajuda está em período de espera
            if (questionNumber < nextRemoveAvailable)
            {
                int rodadasRestantes = nextRemoveAvailable - questionNumber;

                MessageBox.Show(
                    "Você não pode usar a ajuda ainda." + Environment.NewLine +
                    "Espere mais " + rodadasRestantes + " rodada(s)."
                );

                return;
            }

            // Verifica se já foi usada nesta pergunta
            ...
            // Desabilita uma resposta errada
            buttons[wrongButton].Enabled = false;
            buttons[wrongButton].Text = "X";

            // Marca que a ajuda foi usada nesta pergunta
            removeUsedThisQuestion = true;

            // A ajuda ficará disponível novamente após 3 rodadas
